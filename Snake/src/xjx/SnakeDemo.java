...
//	public static long normal_speed = 600000; //debug speed
	...
	public SnakeDemo(){
	    ...
	    Length.setForeground(Color.pink);
		Score.setForeground(Color.pink);
		Time.setForeground(Color.pink);
		Time2.setForeground(Color.pink);    
		Weapon.setForeground(Color.pink); 
		
		for(int i = 0;i < obstacle.length;i++)
		{
			obstacle[i] = new Barrier(obstacle_label.length/obstacle.length);
		}
	    ...
	    //初始化所有砖块
	    brickIcon = new ImageIcon("body.png");
		...
		for(int i = 0; i < 40;i++)
		{
			...
			obstacle_label[i].setOpaque(true);//true :not transparent and false means transparent
		}
	...
	@SuppressWarnings("deprecation")
 	public void HitWall(){//判断是否撞墙
		if(current_direction == "L")
		{
			...
			{
				ifcount = false;
				new AePlayWave("C:\\Users\\16770\\Music\\over.wav").start();
				isrun = false;
